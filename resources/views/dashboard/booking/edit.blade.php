...
@section('content')
  ...
  <div class="container mt-3 d-flex justify-content-center">
    <div class="card rounded-3" style="width: 1100px;">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('booking.update', $booking->getKey()) }}">
          ...
          <div class="mb-3 mt-4 row">
            ...
            <div class="col-sm-10">
              <input type="text" name="no_invoice" class="form-control" id="no_invoice"
                placeholder="Masukkan Nomor Invoice" autocomplete="off" value="{{ old('no_invoice', $booking->no_invoice) }}">
            </div>
            @error('no_invoice')
            ...
            <div class="col-sm-10">
              <select class="form-control" name="pelanggan_id" id="pelanggan_id">
                @foreach ($pelanggans as $pelanggan)
                  <option {{ (old('pelanggan_id', $booking->pelanggan->getKey()) == $pelanggan->id) ? 'selected' : ''  }} value="{{ $pelanggan->getKey() }}">{{ $pelanggan->nama }}</option>
                @endforeach
              </select>
            </div>
            @error('pelanggan_id')
          ...
          <div class="mb-3 mt-4 row">
            ...
            <div class="col-sm-10">
              <input type="text" name="tgl_transaksi" class="form-control" id="tgl_transaksi"
                placeholder="Format: TTTT-BB-HH, contoh: 2022-01-02" autocomplete="off" value="{{ old('tgl_transaksi', $booking->tgl_transaksi) }}">
            </div>
            @error('tgl_transaksi')
            <div class="col-sm-2"></div>
            <div class="text-danger col-sm-10">
          ...
          <div class="mb-3 mt-4 row">
            ...
            <div class="col-sm-10">
              <input type="text" name="keterangan" class="form-control" id="keterangan"
                placeholder="Masukkan Keterangan" autocomplete="off" value="{{ old('keterangan', $booking->keterangan) }}" >
            </div>
            @error('keterangan')
            <div class="col-sm-2"></div>
            <div class="text-danger col-sm-10">
